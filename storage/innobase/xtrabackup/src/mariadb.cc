...
/*========================*/
	const page_size_t&	page_size,	/*!< in: page_size */
	ulint*			max_size)	/*!< out: free space available
						for crypt data */
{
	ulint pageno = 0;
	...
	const page_size_t&	page_size)
{
	...
	byte*		page;
	ulint		maxsize=0;
	os_offset_t	offset = fsp_header_get_crypt_offset(page_size, &maxsize);
	...
	err = os_file_read(read_request, fd, page, 0, UNIV_PAGE_SIZE);

	if (err != DB_SUCCESS) {
		msg("Failed to read page 0");
		return false;
	}
