...
decrypt_init(const char *root)
{
	...
	decrypt_ctxt->nthreads = xtrabackup_encrypt_threads;
...
decrypt_open(ds_ctxt_t *ctxt, const char *path, MY_STAT *mystat)
{
	...
	crypt_file->rest = NULL;
	crypt_file->rest_size = 0;
	crypt_file->rest_len = 0;
...
decrypt_write(ds_file_t *file, const void *buf, size_t len)
{
	...
	if (crypt_file->rest_len > 0) {
		...
		do {
			if (parse_result == XB_CRYPT_READ_INCOMPLETE) {
				crypt_file->rest_size = crypt_file->rest_size * 2;
				crypt_file->rest = (uchar *) my_realloc(
						PSI_NOT_INSTRUMENTED,
						crypt_file->rest,
						crypt_file->rest_size,
						MYF(MY_FAE | MY_ALLOW_ZERO_PTR));
			}

			memcpy(crypt_file->rest + crypt_file->rest_len,
			       buf, MY_MIN(crypt_file->rest_size -
					   crypt_file->rest_len, len));

			parse_result = parse_xbcrypt_chunk(
				thd, crypt_file->rest,
				crypt_file->rest_size, &bytes_processed);

		} while (parse_result == XB_CRYPT_READ_INCOMPLETE &&
			 crypt_file->rest_size < len);

		if (parse_result != XB_CRYPT_READ_CHUNK) {
			msg("decrypt: data is incomplete.\n");
			return 1;
		}
		...
		len -= bytes_processed - crypt_file->rest_len;
		buf += bytes_processed - crypt_file->rest_len;
	...
	if (parse_result == XB_CRYPT_READ_INCOMPLETE && len > 0) {
		...
		memcpy(crypt_file->rest, buf, len);
	}
